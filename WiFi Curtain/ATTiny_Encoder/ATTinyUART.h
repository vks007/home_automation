 ...
 * Log your quesiton about it here: http://blog.podkalicki.com/attiny13-software-uart-debug-logger/
 */
#ifndef ATTinyUART_h
#define ATTinyUART_h
...
//copied from https://github.com/lpodkalicki/blog/blob/master/avr/attiny13/029_log_temperature_lm35_to_software_uart/uart.c
//not sure how does this work though
...
//Prints a long number to the serial output
void uart_putl(long x)
{
  char buff[12] = {0};
  char *p = buff+10;
  buff[11] = '\0';

  //save the -negative flag for later use
  bool flag = false;
  if (x < 0) {
      flag = true;
      x = -x;
  ...
  if (flag) *(p--) = '-'; //If it is a negative no then start printing from one character before the actual no so we do --
  uart_puts((const char *)(p+1)); //The actual no starts from the location *p+1
}

//Prints an unsigned int to the serial output
